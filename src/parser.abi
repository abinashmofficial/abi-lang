...
export class Parser {
  ...
  private classDecl(): Statement {
    const line = this.previous().line;
    const name = this.consume(TokenType.IDENTIFIER, "Expect class name.").value;
    
    let parent: string | null = null;
    if (this.match(TokenType.EXTENDS)) {
      parent = this.consume(TokenType.IDENTIFIER, "Expect parent class name.").value;
    }
    ...
    while (!this.check(TokenType.RBRACE) && !this.isAtEnd()) {
      ...
      this.consume(TokenType.FUNC, "Expect 'func' for method declaration.");
      const methodLine = this.previous().line;
      const methodName = this.consume(TokenType.IDENTIFIER, "Expect method name.").value;

      this.consume(TokenType.LPAREN, "Expect '(' after method name.");
      const params: string[] = [];
      if (!this.check(TokenType.RPAREN)) {
        do {
          params.push(this.consume(TokenType.IDENTIFIER, "Expect parameter name.").value);
  ...
  private tryCatchStatement(): Statement {
    const line = this.previous().line;

    this.consume(TokenType.LBRACE, "Expect '{' after 'try'.");
    const tryBranch = this.block();

    let catchVar: string | null = null;
    let catchBranch: Statement[] | null = null;

    if (this.match(TokenType.CATCH)) {
      if (this.match(TokenType.LPAREN)) {
        catchVar = this.consume(TokenType.IDENTIFIER, "Expect catch variable name.").value;
        this.consume(TokenType.RPAREN, "Expect ')' after catch variable.");
      } else if (this.check(TokenType.IDENTIFIER)) {
        catchVar = this.advance().value;
      }
      this.consume(TokenType.LBRACE, "Expect '{' before catch body.");
      catchBranch = this.block();
    }

    let finallyBranch: Statement[] | null = null;
    if (this.match(TokenType.FINALLY)) {
      this.consume(TokenType.LBRACE, "Expect '{' before finally body.");
